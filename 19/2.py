...
def main(in_string):
    ...
    for bp in blueprints[:3]:
        ...
        while len(to_explore) > 0:
            ...
            if state.geode + ((32-state.minute)*state.geode_rbs) + ((32-state.minute)*(32-state.minute+1))/2 < max_geodes:
                continue
            ...
            if state.ore_rbs > 0 and state.clay_rbs > 0:
                if state.obs >= ((32-state.minute)*top_obs)-((32-state.minute)*state.obs_rbs):
                    pass
                else:
                    can_make_obs_rbs = True
            if state.ore_rbs > 0:
                if state.ore >= ((32-state.minute)*top_ore)-((32-state.minute)*state.ore_rbs):
                    pass

                else:
                    can_make_ore_rbs = True

                if state.clay >= ((32-state.minute)*top_clay)-((32-state.minute)*state.clay_rbs):
                    pass
                else:
            ...
            if can_make_ore_rbs:
                ...
                if state.minute + time_inc < 32:
                    ...
                    potential_geodes = state.geode + ((32-state.minute)*state.geode_rbs)
    ...
    print(np.prod(overall_max_geodes))
