...
def main(in_string):
    ...
    # x, y, timestep, mins
    to_explore = [(start[0], start[1], 0, 0, 0)]
    count = -1
    while len(to_explore) > 0:
        count += 1
        x, y, timestep, mins, targets = to_explore.pop(0)
        if count % 1000 == 0:
            print(x,y,timestep,mins)

        new_targets = targets
        if x == finish[0] and y == finish[1] and targets == 2:
            print(mins)
            break
        elif x == finish[0] and y == finish[1] and targets == 0:
            new_targets = targets + 1

        elif x == start[0] and y == start[0] and targets == 1:
            new_targets = targets + 1
        ...
        if not (y in blizzards and x in blizzards[y]):
            new_state = (x, y, timestep+1, mins+1, new_targets)
            if new_state not in to_explore:
                to_explore.append(new_state)

        # go up
        if not (y-1 in blizzards and x in blizzards[y-1]) and y-1 > 0 and grid[y-1][x] == '.':
            new_state = (x, y-1, timestep+1, mins+1, new_targets)
            if new_state not in to_explore:
                to_explore.append(new_state)

        # go down
        if not (y+1 in blizzards and x in blizzards[y+1]) and y+1 < len(grid) and grid[y+1][x] == '.':
            new_state = (x, y+1, timestep+1, mins+1, new_targets)
            if new_state not in to_explore:
                to_explore.append(new_state)
        ...
        if not (y in blizzards and x-1 in blizzards[y]) and grid[y][x-1] == '.':
            new_state = (x-1, y, timestep+1, mins+1, new_targets)
            if new_state not in to_explore:
                to_explore.append(new_state)
